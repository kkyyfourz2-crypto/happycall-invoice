...
from sheets_integration import build_vendor_db, build_order_db, find_best_vendor, find_matching_order, fetch_sheet
...
WOLMAL_MAP = {
    ...
    '마포르플로스': {'aliases': ['마포르플로스','르플로'], 'addr': ['삼개로23']},
...
def get_sheets_data():
    try:
        ...
        order_db = build_order_db()
        return jsonify({
            'vendors': list(vendor_db.keys()),
...
[결제방식] 기타칸: 원말/웜말/윌말/월말 → "월말", 현금/선불 → "현금", 계좌/이체 → "계좌"

[배송원] "배송원:" 옆 크게 인쇄된 이름. "(연락처: 010-XXXX-XXXX)" 번호 추출.
080-220-9988, 출발지TEL, 도착지TEL은 배송원 번호 아님.

[금액] 운송요금란. 1000원 단위. 6000~40000원.

[수요처] 출발지 상호명과 아래 목록 비교해서 표준명으로:
{vendor_list}

JSON 배열만 반환:
[{{"날짜":"2026-03-30","결제방식":"월말","수요처":"미니쉬치과","출발지":"미니쉬치과","도착지":"이살리는치과 선릉","가격":8000,"배송원":"남명현","배송원전화":"010-4410-0066","입금자명":"","비고":"","uncertain":false}}]

판독 불가만 uncertain:true. 전표 아니면 [] 반환."""
    ...
    try:
        ...
        for item in parsed:
            ...
            # 구글시트 DB로 수요처 교정
            best_vendor = find_best_vendor(
                item.get('수요처', ''), 
                item.get('출발지', ''),
                WOLMAL_MAP
            )
            
            # 구글시트 주문 DB로 출발지/도착지 교정
            depart = item.get('출발지', '')
            arrive = item.get('도착지', '')
            corrected = False
            matched_order = find_matching_order(item.get('날짜', ''), driver, order_db)
            if matched_order:
                if not depart and matched_order.get('depart'):
                    depart = matched_order['depart']
                    corrected = True
                if not arrive and matched_order.get('arrive'):
                    arrive = matched_order['arrive']
                    corrected = True
            ...
            results.append({
                **item,
                ...
                '출발지': depart,
                '도착지': arrive,
                '_uncertain': item.get('uncertain', False) and not corrected,
...
def export_excel():
    ...
    def make_sheet(wb, name, headers, rows):
    ...
    for ri, (name, d) in enumerate(sorted(drivers.items()), 2):
        ...
        for ci, val in enumerate([name, d['월말'], d['월말금액'], d['현금'], d['현금금액'], d['계좌'], d['계좌금액'], tc, ta], 1):
